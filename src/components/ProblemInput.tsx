...
export const ProblemInput: React.FC<ProblemInputProps> = ({
  ...
  return (
    <div className="w-full sakura-card p-6 flex flex-col gap-5 z-10 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          ...
          <div className="relative flex-1 sm:w-80">
            <select
              ...
              className="w-full bg-midnight-950 border border-midnight-700 hover:border-sakura-500/50 text-slate-200 rounded-xl px-3 py-2.5 text-xs font-mono focus:outline-none focus:border-sakura-500 transition-all cursor-pointer shadow-inner"
            >
      ...
      {/* Ghi chú gợi ý theo Model đang chọn */}
      <div className="text-[11px] font-mono text-slate-400 flex items-center gap-1.5 -mt-2">
        <Info className="w-3.5 h-3.5 text-sakura-400/80 shrink-0" />
        {selectedModel === 'gemini-3.8-flash' && (
          <span>
            <b className="text-amber-400">Gemini 3.8 Flash:</b> Ưu tiên cho bài toán phức tạp, cây, đồ thị, quy hoạch động khó. (Giới hạn: 20 lượt/ngày).
          </span>
        )}
        {selectedModel === 'gemini-3.5-flash-lite' && (
          <span>
            <b className="text-sky-400">Gemini 3.5 Flash Lite:</b> Lựa chọn cân bằng tối ưu giữa tốc độ & độ chính xác cao. (Giới hạn: 500 lượt/ngày).
          </span>
        )}
        {selectedModel === 'gemini-3.1-flash-lite' && (
          <span>
            <b className="text-emerald-400">Gemini 3.1 Flash Lite:</b> Phù hợp bài cơ bản, mảng 1D, chạy siêu nhanh tiết kiệm lượt dùng. (Giới hạn: 500 lượt/ngày).
          </span>
        )}
      </div>
    </div>
